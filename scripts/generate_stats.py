...
def svg_card(today, cells, days, weeks):
    ...
    for k in range(1, 4):
        x = PAD + k * cw
        A(f'<line x1="{x:.0f}" y1="{grid_top + 4}" x2="{x:.0f}" y2="{grid_top + 2 * row_h - 20}" '
          f'stroke="{BORDER}"/>')
    for i, (num, label) in enumerate(cells):
        r, c = divmod(i, 4)
        x = PAD + c * cw
        ny = grid_top + r * row_h + 34
        A(f'<text class="mono num" x="{x:.0f}" y="{ny}">{esc(num)}</text>')
        A(f'<text class="mono lbl" x="{x:.0f}" y="{ny + 24}">{esc(label)}</text>')
    ...
    lx = W - PAD - (5 * 14 + 60)
    A(f'<text class="mono mon" x="{lx}" y="{months_y}">less</text>')
    for i in range(5):
        A(f'<rect x="{lx + 34 + i * 14}" y="{months_y - 10}" width="11" height="11" rx="2.5" '
          f'fill="{LEVELS[i]}"/>')
    A(f'<text class="mono mon" x="{lx + 34 + 5 * 14 + 6}" y="{months_y}">more</text>')

    # heatmap — key is (weekday, week); x follows the week, y follows the weekday
